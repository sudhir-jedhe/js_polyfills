...
export const bestHands = (hands) => {
  ...
  for (let i = 0; i < handz.length; i++) {
    ...
    for (let y = 0; y < handz[i].length; y++) {
      ...
      if (handz[i][y].length === 3) handz[i][y] = [10, handz[i][y][2]];
  ...
  for (let i = 0; i < ranked.length; i++) {
    ...
    for (let card of ranked[i][1]) {
      ...
      if (colors[card[1]] !== undefined) colors[card[1]] += 1;
      else colors[card[1]] = 1;
    }
    ...
    for (let color in colors) {
      if (colors[color] === 5) ranked[i][0] = 5;
    }

    console.log(colors);

    if (straight === 5) {
      if (ranked[i][0] === 5) ranked[i][0] = 8;
      else if (ranked[i][0] < 5) ranked[i][0] = 4;
    }
  }

  ranked = ranked
    .map((r) => {
      if (r[0] === 2) r[2] = r[2].sort((a, b) => b - a);
    ...
    if (winners[0][0] === ranked[i][0]) {
      switch (winners[0][0]) {
        case 0:
          ...
          if (counter === 5) winners.push(ranked[i]);
          break;
        case 1:
          if (winners[0][2][0] < ranked[i][2][0]) winners[0] = ranked[i];
            ...
            if (counter === 3) winners.push(ranked[i]);
          }
          break;
        case 2:
          if (winners[0][2][0] < ranked[i][2][0]) winners[0] = ranked[i];
          else if (winners[0][2][0] === ranked[i][2][0])
            if (winners[0][2][1] < ranked[i][2][1]) winners[0] = ranked[i];
            else if (winners[0][2][1] === ranked[i][2][1])
              if (winners[0][1][0] < ranked[i][1][0]) winners[0] = ranked[i];
          ...
          if (winners[0][2][0] < ranked[i][2][0]) winners[0] = ranked[i];
            ...
            for (y = 0; y < winners[0][1].length; y++) {
              if (winners[0][1][0][0] < ranked[i][1][0][0])
                winners[0] = ranked[i];
              else if (winners[0][1][0][0] === ranked[i][1][0][0]) counter += 1;
            }
            if (counter === 2) winners.push(ranked[i]);
          }
          break;
        case 4:
          if (winners[0][1][0][0] < ranked[i][1][0][0]) winners[0] = ranked[i];
