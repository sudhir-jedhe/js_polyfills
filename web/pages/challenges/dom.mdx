...
class NodeStore {
    constructor() {
        // Initialize the store structure using a Map
        this.store = new Map();
    }

    set(node, value) {
        // Associate the node with the value
        this.store.set(node, value);
    }

    get(node) {
        // Retrieve the value associated with the node
...
// nodeStore.set(myNode, 'Some value');
// console.log(nodeStore.get(myNode)); // Output: 'Some value'
// console.log(nodeStore.has(myNode)); // Output: true
// console.log(nodeStore.has({})); // Output: false (different object)
